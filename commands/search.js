...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    embed.setDescription(`${maxTracks.map((track, i) => `**${i + 1}**. ${track.title} | \`${track.author}\``).join('\n')}\n\nBir şarkı seçin: **1** ile **${maxTracks.length}** yaz gönder veya **iptal** yaz ve seçimi iptal et.⬇️`)
    ...
    collector.on('collect', async (query) => {
      if (["iptal"].includes(query.content)) {
      ...
      if (!value || value <= 0 || value > maxTracks.length) return interaction.reply({ content: `Hata: Bir şarkı seçin **1** ile **${maxTracks.length}** ve gönder yazın veya **iptal** yazın ve seçimi iptal edin. ❌`, ephemeral: true }).catch(e => { })
